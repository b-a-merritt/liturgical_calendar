:- module(util, [
    ...
    next_sunday/6, 
    nearest_sunday/6,
    sunday_on_or_after/6,
    previous_sunday_before/6
]).

:- use_module(library(date)).

weekday_atom(N, Name) :-
    nth1(N, ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'], Name).

date_stamp(Y, M, D, T) :-
    date_time_stamp(date(Y, M, D, 0,0,0,0, 'UTC', -), T).

days_after(Year, Month, Day, Numdays, Newyear, Newmonth, Newday) :-
    date_stamp(Year, Month, Day, Stamp),
...
sunday_on_or_after(Y0, M0, D0, Y, M, D) :-
    day_of_the_week(date(Y0,M0,D0), W0),
    ( W0 =:= 7
    -> Y=Y0, M=M0, D=D0
    ;  next_sunday(Y0,M0,D0, Y,M,D)
    ).

previous_sunday_before(Y0, M0, D0, Y, M, D) :-
    day_of_the_week(date(Y0,M0,D0), W0),
    Offset is W0,
    days_after(Y0, M0, D0, -Offset, Y, M, D).
